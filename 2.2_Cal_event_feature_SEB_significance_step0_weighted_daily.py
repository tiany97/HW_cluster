...
for i_label in [3]:
    ...
    for i_random in range(1000):
        all_ano = np.zeros((15330,720,1440))
        temp = label_map[i_label]
        for ii, i in enumerate(all_ano):
            i[(land_mask<0.1)|(LAT_era5<-60)]=np.nan
            i[temp<1] = np.nan
            if mon_era5_all[ii] in [3,4,5,6,7,8,9,10,11]:
                i[LAT_era5<-23.5]=  np.nan
            if mon_era5_all[ii] in [1,2,3,4,5,9,10,11,12]:
                i[LAT_era5>23.5]=  np.nan
    ...
    indices_xr.to_netcdf('/Net/Groups/BGI/scratch/yt/result/Heat_wave_3D_99th_connect6_025_degree/strict_significance_N_'+str(i_label)+'daily'+vername+'_weighted.nc')
