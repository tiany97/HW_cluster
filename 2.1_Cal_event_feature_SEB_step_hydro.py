...
def read_data_025(varname, varname1):
    ...
    return var_all
    #%%
# swvl1 = read_data_025('swvl1','an')
# swvl1_ano = remove_seasonal(swvl1)
# del swvl1
# get_HW_ano_all('swvl1',swvl1_ano, HW_date,HW_lat,HW_lon)
# del swvl1_ano

# tcc = read_data_025('tcc','an')
# tcc_ano = remove_seasonal(tcc)
# del tcc
# get_HW_ano_all('tcc',tcc_ano, HW_date,HW_lat,HW_lon)
# del tcc_ano
#%%

tp = read_data_025('tp','fc')
tp[tp<0]=0
tp = np.nanmean(tp,axis=0)

pev = read_data_025('pev','fc')
pev = -pev
pev[pev<0]=np.nan
pev = np.nanmean(pev,axis=0)
...
for i_lat,i_lon in zip(HW_lat,HW_lon):
    HW_ano.append(AI[i_lat,i_lon])
var = 'AI'
with open(r'/Net/Groups/BGI/scratch/yt/result/Heat_wave_3D_99th_connect6_025_degree/HW_'+var+'_ano_6days_all_235.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerows(HW_ano)
    print(var+'end')
...
#     var_temp=[]
#     for i_temp in var:
#         i_temp = np.concatenate((i_temp[:,720:] ,i_temp[:,:720]),axis=1 )
...
#     z = read_data_025_1('z500','z500_daily.fc.era5.1440.720.'+str(i_year)+'.nc','z',LAT_era5_025,LON_era5_025,i_year)
#     z500.extend(z)
# z500 = np.array(z500)
# print(z500.shape)
# z500_ano = remove_seasonal(z500)
# del z500
# get_HW_ano_all('z500',z500_ano, HW_date,HW_lat,HW_lon)
# del z500_ano